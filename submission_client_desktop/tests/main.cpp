...
TEST_F(SubmissionClientTest, SubmitJobSendsPostRequestToCorrectUrl) {
    ...
    ApiClient apiClient(g_dispatchServerUrl, g_apiKey, g_caCertPath, true, std::move(mock_cpr_api));
...
TEST_F(SubmissionClientTest, SubmitJobDisablesSslVerificationWhenNoCaPathProvided) {
    ...
    ApiClient apiClient(g_dispatchServerUrl, g_apiKey, g_caCertPath, false, std::move(mock_cpr_api));
...
TEST_F(SubmissionClientTest, ListAllEnginesCorrectlyParsesJsonResponse) {
    auto mock_cpr_api = std::make_unique<MockCprApi>();
