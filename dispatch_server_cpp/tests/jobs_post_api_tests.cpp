...
TEST_F(ApiTest, CompletedJobIsNotReassigned) {
    // 1. Create a job
    nlohmann::json job_payload = {
    ...
    // 2. Manually set the job's status to "completed" in the database
    {
        std::lock_guard<std::mutex> lock(jobs_mutex);
...
TEST_F(ApiTest, AssignedJobIsNotReassigned) {
    ...
    // 2. Manually set the job's status to "assigned" in the database
    {
        std::lock_guard<std::mutex> lock(jobs_mutex);
    ...
    // 4. Try to assign the assigned job
    nlohmann::json assign_payload = {
        {"engine_id", "engine-123"}
    };
    auto res_assign = client->Post("/assign_job/", admin_headers, assign_payload.dump(), "application/json");
