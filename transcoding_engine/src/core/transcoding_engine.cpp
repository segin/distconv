...
void TranscodingEngine::heartbeat_loop() {
    while (running_.load()) {
        ...
        shutdown_cv_.wait_for(lock, std::chrono::seconds(config_.heartbeat_interval_seconds), [this] {
            return !running_.load();
        });
    }
}
...
void TranscodingEngine::benchmark_loop() {
    while (running_.load()) {
        ...
        std::unique_lock<std::mutex> lock(shutdown_mutex_);
        shutdown_cv_.wait_for(lock, std::chrono::minutes(config_.benchmark_interval_minutes), [this] {
            return !running_.load();
        });
    }
}
...
void TranscodingEngine::main_job_loop() {
    while (running_.load()) {
        ...
        std::unique_lock<std::mutex> lock(shutdown_mutex_);
        shutdown_cv_.wait_for(lock, std::chrono::seconds(config_.job_poll_interval_seconds), [this] {
            return !running_.load();
        });
    }
}
