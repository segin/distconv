...
// Test 151: The engine correctly configures SSL options
TEST_F(ImplementationTest, SSLOptionsConfiguration) {
    CprHttpClient http_client;

    // Test that we can set SSL options without crashing
    http_client.set_ssl_options("/tmp/cert.pem", false);

    // We cannot easily inspect the internal state without mocking CPR or exposing implementation details.
    // However, we can verify that the client remains usable and doesn't crash when making requests with these options.

    // This request will likely fail due to network, but it should exercise the SSL options creation logic.
    auto response = http_client.get("http://localhost:1234");

    // The call should complete (even if with failure)
    EXPECT_TRUE(response.status_code == 0 || response.status_code >= 100);
}
